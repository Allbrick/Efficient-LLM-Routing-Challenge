...
from routing_stack.training.calibration_reports import build_sufficiency_calibration
from routing_stack.training.outcome_matrix import merge_outcome_training


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate model sufficiency calibration report assets.")
    ...
    calibration_df, summary_df = build_sufficiency_calibration(
        router,
        train_df,
        specs_df,
        bins=args.bins,
    ...
    calibration_df.to_csv(output / "sufficiency_calibration.csv", index=False, encoding="utf-8")
    summary_df.to_csv(output / "sufficiency_calibration_summary.csv", index=False, encoding="utf-8")
    payload = {
        "files": [
    ...
    (output / "sufficiency_calibration_summary.json").write_text(
